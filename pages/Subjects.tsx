...
export const Subjects: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!user) return;
    try {
      await BackendService.saveSubject(user, editingSubject);
      setMessage({ type: 'success', text: t('successSaved') });
      setIsModalOpen(false);
      loadData();
    } catch (err) {
      setMessage({ type: 'error', text: 'Error saving subject' });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await BackendService.deleteSubject(user, id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">{t('manageSubjects')}</h2>
        ...
        <table className="w-full text-start">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-start">{t('code')}</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {filtered.map(sub => (
              <tr key={sub.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm text-gray-500">{teachers.find(t => t.id === sub.teacherId)?.name || '-'}</td>
                <td className="px-6 py-4 flex gap-2">
                  <button onClick={() => openModal(sub)} className="text-blue-600 hover:text-blue-800"><Edit size={18} /></button>
                  <button onClick={() => handleDelete(sub.id)} className="text-red-600 hover:text-red-800"><Trash2 size={18} /></button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold">{editingSubject.id ? t('edit') : t('add')}</h3>
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                ...
                <input required className="w-full border rounded-lg p-2"
                  value={editingSubject.name || ''} onChange={e => setEditingSubject({...editingSubject, name: e.target.value})}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('code')}</label>
                <input required className="w-full border rounded-lg p-2"
                  value={editingSubject.code || ''} onChange={e => setEditingSubject({...editingSubject, code: e.target.value})}
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Teacher</label>
                <select className="w-full border rounded-lg p-2"
                  value={editingSubject.teacherId || ''} onChange={e => setEditingSubject({...editingSubject, teacherId: e.target.value})}
                >
